...
bool SampleOnnxMNIST::build() {
  ...
  auto constructed =
      constructNetwork(builder, network, config, parser, timingCache);
  if (!constructed) {
    return false;
  }
  ...
  if (timingCache != nullptr && !mParams.timingCacheFile.empty()) {
    samplesCommon::updateTimingCacheFile(sample::gLogger.getTRTLogger(),
                                         mParams.timingCacheFile,
                                         timingCache.get(), *builder);
  }
...
bool SampleOnnxMNIST::constructNetwork(
  ...
  auto parsed = parser->parseFromFile(
      samplesCommon::locateFile(mParams.onnxFileName, mParams.dataDirs).c_str(),
  ...
  if (mParams.timingCacheFile.size()) {
    timingCache = samplesCommon::buildTimingCacheFromFile(
        sample::gLogger.getTRTLogger(), *config, mParams.timingCacheFile);
  }
...
bool SampleOnnxMNIST::processInput(
  ...
  samplesCommon::readPGMFile(
      samplesCommon::locateFile(std::to_string(mNumber) + ".pgm",
                                mParams.dataDirs),
...
bool SampleOnnxMNIST::verifyOutput(
  ...
  for (int i = 0; i < outputSize; i++) {
    ...
    sample::gLogInfo << " Prob " << i << "  " << std::fixed << std::setw(5)
                     << std::setprecision(4) << output[i] << " " << "Class "
                     << i << ": "
...
initializeSampleParams(const samplesCommon::Args &args) {
  ...
  if (args.dataDirs.empty()) // Use default directories if user hasn't provided
                             // directory paths
  {
...
void printHelpInfo() {
  ...
  std::cout << "--datadir          Specify path to a data directory, "
               "overriding the default. This option can be used "
               "multiple times to add multiple directories. If no data "
               "directories are given, the default is to use "
               "(data/samples/mnist/, data/mnist/)"
            << std::endl;
  std::cout << "--useDLACore=N     Specify a DLA engine for layers that "
               "support DLA. Value can range from 0 to n-1, "
               "where n is the number of DLA engines on the platform."
            << std::endl;
  std::cout << "--timingCacheFile  Specify path to a timing cache file. If it "
