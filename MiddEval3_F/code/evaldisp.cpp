...
void evaldisp(CFloatImage disp, CFloatImage gtdisp, CByteImage mask, float badthresh, int maxdisp, int rounddisp)
{
    ...
    int invalid = 0;
    float serr = 0;
    double serr2 = 0; // sum of squared errors for RMS
    ...
    float rms = (n - invalid > 0) ? sqrt(serr2 / (n - invalid)) : 0;
    //printf("mask  bad%.1f  invalid  totbad   avgErr    rms\n", badthresh);
    printf("%4.1f  %6.2f  %6.2f   %6.2f  %6.2f  %6.2f\n",   100.0*n/(width * height), 
	   badpercent, invalidpercent, totalbadpercent, avgErr, rms);
}
