...
void readVizRange(char *calibfile, float &dmin, float &dmax) {
  ...
  if (fp != NULL) {
    while (fgets(line, sizeof line, fp) != NULL) {
      if (sscanf(line, " vmin= %f", &f) == 1)
        dmin = f;
      if (sscanf(line, " vmax= %f", &f) == 1)
        dmax = f;
    }
    fclose(fp);
  } else
...
void jet(float x, int &r, int &g, int &b) {
  ...
  if (x > 1)
    x = 1.05;
  x = x / 1.15 +
      0.1; // use slightly asymmetric range to avoid darkest shades of blue.
...
int main(int argc, char *argv[]) {
  ...
  try {
    if (calibfile != NULL)
      readVizRange(calibfile, dmin,
                   dmax); // if calibfile given, extract dmin, dmax from that
