...
void GPUMonitoringNode::pollMetrics() {
  ...
  metrics_msg.memory_gb = static_cast<float>(memory_info.used) / (1024.0f * 1024.0f * 1024.0f);
  metrics_msg.power_w = static_cast<float>(power_usage) / 1000.0f;
  metrics_msg.min_memory_gb = static_cast<float>(min_mem_) / 1024.0f;
  metrics_msg.max_memory_gb = static_cast<float>(max_mem_) / 1024.0f;
  metrics_msg.min_power_w = static_cast<float>(min_power_) / 1000.0f;
  metrics_msg.max_power_w = static_cast<float>(max_power_) / 1000.0f;
  metrics_msg.avg_memory_gb = static_cast<float>(mem_sum_ / count_polls_) / 1024.0f;
