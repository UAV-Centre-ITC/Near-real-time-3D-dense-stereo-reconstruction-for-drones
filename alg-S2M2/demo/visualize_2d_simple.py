...
from s2m2.core.utils.vis_utils import (
    visualize_stereo_results_2d,
    custom_visualize_stereo_results_2d,
)
...
def get_args_parser():
    ...
    # --- ADDED ARGUMENTS ---
    parser.add_argument(
        "--left_img", type=str, required=True, help="path to the left input image"
    )
    parser.add_argument(
        "--right_img", type=str, required=True, help="path to the right input image"
    )
    # -----------------------

    parser.add_argument(
        "--model_type", default="S", type=str, help="select model type: S,M,L,XL"
    )
    parser.add_argument(
        "--num_refine", default=3, type=int, help="number of local iterative refinement"
    )
    parser.add_argument(
        "--torch_compile", action="store_true", help="apply torch_compile"
    )
    parser.add_argument(
        "--allow_negative",
        action="store_true",
...
def main(args):
    ...
    # load stereo model
    model = load_model(
        weights_path, args.model_type, not args.allow_negative, args.num_refine, device
    )
