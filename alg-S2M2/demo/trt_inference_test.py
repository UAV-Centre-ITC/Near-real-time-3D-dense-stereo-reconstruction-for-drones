...
def main(args):
    ...
    # to torch tensor
    input_left = (
        (torch.from_numpy(left).permute(-1, 0, 1).unsqueeze(0)).to(device).contiguous()
    )
    input_right = (
        (torch.from_numpy(right).permute(-1, 0, 1).unsqueeze(0)).to(device).contiguous()
    )
    ...
    print(
        f"torch avg inference time:{(curr_time) / T / 1000}, FPS:{1000 * T / (curr_time)}"
    )
